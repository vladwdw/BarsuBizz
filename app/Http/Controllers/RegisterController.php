...
class RegisterController extends Controller
{
    public function register(Request $request)
    {
        ...
        try {
            ...
            mail($user->email,"Подтверждение почты",
            "<!DOCTYPE html PUBLIC 
            <html>
              <head>
                                            ...
                                            <tr>
                                              <td style='line-height: 24px; font-size: 16px; width: 100%; margin: 0; padding: 40px;' align='left' bgcolor='#ffffff'>
                                                ...
                                                <p class='' style='line-height: 24px; font-size: 16px; width: 100%; margin: 0;' align='left'>
                                                 Что бы пользоваться нашим сайтом нужно подтвердить адресс электронной почты. Для этого нажмите кнопку ниже.
                                                </p>
                                                <table class='s-4 w-full' role='presentation' border='0' cellpadding='0' cellspacing='0' style='width: 100%;' width='100%'>
                                                  <tbody>
                                                    <tr>
    ...
    public function verifyEmail($token){
        ...
        if(isset($verifiedUser)){
            if(!$verifiedUser->email_verified_at){
                ...
                $verifiedUser->save();
                return redirect('login')->with('success','Вы подтвердили свой адрес электронной почты');
            }
            else{
                return redirect()->back()->with('info','Произошла ошибка при подтверждении адреса email');
            }
        }
        else{
            return redirect('/login');
